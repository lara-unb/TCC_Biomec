...
def getKeypointsPixels(keypoints_xy, f_height, mmppx=1, mmppy=1):
    ...
    for i in range(len(keypoints_ji)):
        keypoints_ji[i] = list(
            getPixel(keypoints_xy[i], f_height, mmppx, mmppy))
    return keypoints_ji
...
def getKeypointsCoord(keypoints_ji, f_height, mmppx=1, mmppy=1):
    ...
    for i in range(len(keypoints_xy)):
        keypoints_xy[i] = list(
            getCoord(keypoints_ji[i], f_height, mmppx, mmppy))
    return keypoints_xy
...
def drawAngle(img, O, angle, thickness=10, textsize=1):
    ...
    cv2.putText(img, str(int(round(angle))), (O[0]-round(thickness/2), O[1]-round(
        thickness/2)), font, textsize, (1, 0, 0), 2, cv2.LINE_AA)
    return img
...
def showAnglePlot(A, B, O):
    ...
    plt.text(O[0], O[1], "{}".format(round(angle)),
...
def inverseKinematicsRowing(keypoints, anatomical='False'):
    """ Expected joint order
    if orientation == "Sagittal Right":
        joints_order = ["Right Ankle", "Right Knee", "Right Hip", "Right Shoulder", "Right Elbow", "Right Wrist"]
    else:
...
def fowardKinematicsRowing(root_xy, angles, distances, orientation="Sagittal Left"):
    ...
    for i in range(1, 6):
        if i == 1:
            ...
            keypointsFK_xy[i, 0] = keypointsFK_xy[i-1, 0] - \
                (sign)*distances[i-1]*math.cos(arg)
            keypointsFK_xy[i, 1] = keypointsFK_xy[i-1, 1] + \
                distances[i-1]*math.sin(arg)
        elif i == 2:
...
def showRowingChainAnglesImage(frame, keypoints_ji, textsize=0.5, anatomical='False'):
    """ Expected joint order
    if orientation == "Sagittal Right":
        joints_order = ["Right Ankle", "Right Knee", "Right Hip", "Right Shoulder", "Right Elbow", "Right Wrist"]
    else:
        joints_order = ["Left Ankle", "Left Knee", "Left Hip", "Left Shoulder", "Left Elbow", "Left Wrist"]
    """
    angles = inverseKinematicsRowing(keypoints_ji, anatomical)
    for i in range(5):
        O = keypoints_ji[i]
        B = keypoints_ji[i+1]
        frame = drawLine(frame, O, B)
...
def showRowingChainAnglesPlot(keypoints_xy, anatomical='False'):
    """ Expected joint order
    if orientation == "Sagittal Right":
        joints_order = ["Right Ankle", "Right Knee", "Right Hip", "Right Shoulder", "Right Elbow", "Right Wrist"]
    else:
        joints_order = ["Left Ankle", "Left Knee", "Left Hip", "Left Shoulder", "Left Elbow", "Left Wrist"]
    """
    angles = inverseKinematicsRowing(keypoints_xy, anatomical)
    ...
    for i in range(5):
        ...
        plt.text(O[0], O[1], "{}".format(int(round(angles[i]))),
                 bbox=dict(facecolor='red', alpha=0.5))
        plt.gca().set_aspect('equal', adjustable='box')
    plt.grid()
    plt.show()
...
def showRowingChainDistancesPlot(keypoints_xy, plot_path=None, show_plot=True):
    ...
    for i in range(5):
        ...
        plt.plot([A[0], B[0]], [A[1], B[1]], 'ro-', color="black")
        circle1 = plt.Circle((A[0], A[1]), 1, color='r')
        plt.text(O[0], O[1], "{}".format(int(round(distances[i]))),
...
def showRowingChainAnglesDistancesPlot(keypoints_xy, anatomical='False'):
    """ Expected joint order
    if orientation == "Sagittal Right":
        joints_order = ["Right Ankle", "Right Knee", "Right Hip", "Right Shoulder", "Right Elbow", "Right Wrist"]
    else:
    ...
    for i in range(5):
        ...
        plt.text(O[0], O[1], "{}".format(int(round(distances[i]))),
                 bbox=dict(facecolor='blue', alpha=0.5))
        plt.text(A[0], A[1], "{}".format(int(round(angles[i]))),
                 bbox=dict(facecolor='red', alpha=0.5))
        plt.gca().set_aspect('equal', adjustable='box')
    plt.grid()
    plt.show()
